...
class ProduksiController extends Controller
{
    public function workerIndex(string $type)
    {
        ...
        return view('produksi.workers.index', ['workers' => Worker::where('type', $this->getWorkerTypeInt($type))->latest()->paginate(10), 'type' => $type, 'title' => ucfirst($type).' Workers', 'can' => $this->workerPermissions(), 'flash' => ['success' => session('success'), 'error' => session('error')]]);
    }
    ...
    public function setoranIndex(Request $request)
    {
        ...
        $query = Produksi::with(['potong', 'size', 'jahit', 'qc', 'item'])
