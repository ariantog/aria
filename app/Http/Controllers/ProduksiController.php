...
class ProduksiController extends Controller
{
    ...
    public function workerUpdate(Request $request, Worker $worker) { Gate::authorize(Worker::getPermissions()['edit']); $type = (string) $request->route()->parameter('type'); if ($worker->type !== $this->getWorkerTypeInt($type)) abort(404); $request->validate(['name' => 'required|string|max:255']); $worker->update(['name' => trim($request->name)]); return back()->with('success', 'Worker updated.'); }
    public function workerDestroy(Request $request, Worker $worker) { Gate::authorize(Worker::getPermissions()['delete']); $type = (string) $request->route()->parameter('type'); if ($worker->type !== $this->getWorkerTypeInt($type)) abort(404); $worker->delete(); return back()->with('success', 'Worker deleted.'); }
    ...
    public function store(StoreProduksiRequest $request) { Gate::authorize(Produksi::getPermissions()['create']); DB::transaction(function () use ($request) { foreach ($request->items as $item) { Produksi::create(['temp_name' => $item['name'], 'size_id' => $item['size_id'], 'quantity' => $item['qty'], 'customer' => $item['customer'] ? strtoupper($item['customer']) : null, 'warna' => $item['warna'] ? strtoupper($item['warna']) : null, 'potong_id' => $request->potong_id, 'potong_date' => $request->date, 'surat_jalan_potong' => $request->surat_jalan_potong, 'user_id' => $request->user()->id, 'status' => Produksi::STATUS_PRODUKSI]); } }); return redirect()->route('produksi.index')->with('success', 'Production records created.'); }
    ...
    private function getWorkerTypeInt(string $type): int { return match ($type) { 'potong' => Worker::TYPE_POTONG, 'jahit' => Worker::TYPE_JAHIT, 'qc' => Worker::TYPE_QC, 'pritil' => Worker::TYPE_PRITIL, default => abort(404) }; }
