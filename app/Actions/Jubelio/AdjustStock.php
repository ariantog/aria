...
class AdjustStock
{
    ...
    {
        if ($side === 1) {
            if ($transaction->submit_a_count > 0) throw new \RuntimeException('Side A already attempted.');
            $transaction->increment('submit_a_count');
        } else {
            if ($transaction->submit_b_count > 0) throw new \RuntimeException('Side B already attempted.');
        ...
        try {
            ...
            if ($response->successful()) {
                ...
                if ($side === 1) $transaction->update(['a_submit_by' => Auth::id(), 'a_reference_id' => $result['id'] ?? null]);
                else $transaction->update(['b_submit_by' => Auth::id(), 'b_reference_id' => $result['id'] ?? null]);
                DB::commit();
