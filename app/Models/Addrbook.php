...
class Addrbook extends Model
{
    ...
    public static function getPermissions(?string $type = null): array
    {
        ...
        foreach (self::getTypes() as $t) {
            ...
            if ($t['slug'] === 'bank') $typePermissions['hidden-balance'] = "{$group}-hidden-balance";
            if ($type === $t['slug']) return $typePermissions;
            ...
            if (isset($typePermissions['hidden-balance'])) $permissions["{$t['slug']}-hidden-balance"] = $typePermissions['hidden-balance'];
        }
        return $permissions;
    }
    ...
    public function scopeCustomer(Builder $query): Builder { return $query->where('type', AddrbookType::Customer->value); }
    public function scopeWarehouse(Builder $query): Builder { return $query->where('type', AddrbookType::Warehouse->value); }
    public function scopeAccount(Builder $query): Builder { return $query->where('type', AddrbookType::Account->value); }

    public function getTypeNameAttribute(): string { return $this->type instanceof AddrbookType ? $this->type->label() : 'Other'; }
    public function getTypeSlugAttribute(): string { return $this->type instanceof AddrbookType ? $this->type->slug() : 'other'; }

    public function stat() { return $this->hasOne(AddrbookStat::class, 'addrbook_id'); }
    public function dailies() { return $this->hasMany(AddrbookDaily::class, 'addrbook_id'); }
    public function items(): BelongsToMany { return $this->belongsToMany(Item::class, 'warehouse_items', 'warehouse_id', 'item_id')->withPivot('quantity')->withTimestamps(); }
    public function operation() { return $this->belongsTo(Operation::class, 'operation_id'); }
}
