...
export default function Cash({ bankList, ppn_rate, type }: Props) {
    ...
    const config = {
        ...
        endpoint: isCashIn ? transactions.cashIn.store.url() : transactions.cashOut.store.url(),
    ...
    const grandTotal = data.items.reduce((sum, item) => sum + Number(item.total || 0), 0);
        ...
        if (e.key === 'Enter') {
            if (field === 'customer') {
                ...
                if (field === 'invoice_number') {
                    ...
                    setData('items', [
                        ...data.items,
                        { id: newId, customer_id: '', customer: null, invoice_number: '', note: '', total: 0 }
                    ]);
            ...
            <form onSubmit={submit} className="p-4 sm:p-6 lg:p-8 max-w-[1400px] mx-auto">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-50">{config.title}</h2>
                            <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">{config.description}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <Button type="submit" disabled={processing} className={cn(config.submitColor, "text-white min-w-[140px]")}>
                            {processing ? (
                ...
                <div className="space-y-6">
                        ...
                        <CardContent className="p-0">
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm text-left">
                                    <thead className="text-xs font-medium text-zinc-500 bg-zinc-50 dark:bg-zinc-900/50 uppercase tracking-wider">
                                        <tr>
                                            <th className="px-4 py-3 min-w-[300px]">{config.sourceLabel}</th>
                                            <th className="px-4 py-3 min-w-[200px]">Invoice (Opt)</th>
                                            <th className="px-4 py-3 min-w-[250px]">Note</th>
                                            <th className="px-4 py-3 text-right min-w-[150px]">Total Amount</th>
                                        ...
                                        {data.items.map((item, index) => (
                                            <tr key={item.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/50 transition-colors">
                                                <td className="px-4 py-3">
                                                    <AsyncCombobox
                                                        ref={el => setRef(item.id, 'customer', el)}
                                                        ...
                                                        onSelect={() => focusField(item.id, 'invoice_number')}
                                                    />
                                                    {errors[`items.${index}.customer_id`] && (
                                                        <p className="text-[10px] text-red-500 mt-1">{errors[`items.${index}.customer_id`]}</p>
                                                    )}
                                                </td>
                                                <td className="px-4 py-3">
                                                    <Input
                                                        ref={el => setRef(item.id, 'invoice_number', el)}
                                                        value={item.invoice_number}
                                                        onChange={e => updateItem(item.id, 'invoice_number', e.target.value)}
                                                        onKeyDown={e => handleKeyDown(e, item.id, 'invoice_number')}
                                                ...
                                                <td className="px-4 py-3">
                                                    <div className="relative">
                                                        ...
                                                        <Input
                                                            ref={el => setRef(item.id, 'total', el)}
                                                            type="number"
                                                            value={item.total}
                                                            onChange={e => updateItem(item.id, 'total', Number(e.target.value))}
                                                            onKeyDown={e => handleKeyDown(e, item.id, 'total')}
                                    ...
                                    <tfoot className="bg-zinc-50 dark:bg-zinc-900/50 font-bold border-t">
                                        <tr>
                                            <td colSpan={3} className="px-4 py-4 text-right text-zinc-500 font-medium">GRAND TOTAL</td>
                    ...
                    <Alert className="bg-blue-50 border-blue-100 dark:bg-blue-900/10 dark:border-blue-900/20">
                        <Info className="h-4 w-4 text-blue-600" />
                        <AlertTitle className="text-blue-800 dark:text-blue-300">Transaction Info</AlertTitle>
                        <AlertDescription className="text-blue-700/80 dark:text-blue-300/80 text-xs">
                            Each row will be saved as a separate transaction record. If an invoice reference is provided, it will be linked to this payment.
                        </AlertDescription>
                    </Alert>
                </div>
            </form>
