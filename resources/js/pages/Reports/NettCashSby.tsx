...
export default function NettCashSby({
    customerList,
    resellerList,
    bankList,
    customerReport,
    ...
    const sumValues = (obj: Record<number, number>) =>
        ...
        <div className="space-y-4">
            ...
            <div className="rounded-md border">
                <Table>
                            ...
                            list.map((item) => (
                                ...
                                >
                                    ...
                                    <TableCell className="text-right">
                                        {formatCurrency(
                                            report.cashIn[item.id] || 0,
                                        )}
                                    </TableCell>
                                    <TableCell className="text-right">
                                        {formatCurrency(
                                            report.cashOut[item.id] || 0,
                                        )}
                                    </TableCell>
                                    <TableCell className="text-right">
                                        {formatCurrency(
                                            report.sell[item.id] || 0,
                                        )}
                                    </TableCell>
                                    <TableCell className="text-right">
                                        {formatCurrency(
                                            report.return[item.id] || 0,
                                        )}
                                    </TableCell>
                                </TableRow>
                            ))
                    ...
                    <TableFooter>
                        <TableRow className="bg-zinc-50 font-semibold dark:bg-zinc-900">
                            <TableCell>Total</TableCell>
                            <TableCell className="text-right">
                                {formatCurrency(sumValues(report.cashIn))}
                            </TableCell>
                            <TableCell className="text-right">
                                {formatCurrency(sumValues(report.cashOut))}
                            </TableCell>
                            <TableCell className="text-right">
                                {formatCurrency(sumValues(report.sell))}
                            </TableCell>
                            <TableCell className="text-right">
                                {formatCurrency(sumValues(report.return))}
                            </TableCell>
                        </TableRow>
                        <TableRow className="border-t-2 font-bold">
                            <TableCell>Nett</TableCell>
                            <TableCell className="text-right text-emerald-600 dark:text-emerald-400">
                                {formatCurrency(report.nettCash)}
                            </TableCell>
                            <TableCell></TableCell>
                            <TableCell className="text-right text-blue-600 dark:text-blue-400">
                                {formatCurrency(report.nettSell)}
                            </TableCell>
                            <TableCell></TableCell>
                        </TableRow>
                    </TableFooter>
    ...
    const totalCashIn =
        sumValues(customerReport.cashIn) + sumValues(resellerReport.cashIn);
    const totalCashOut =
        sumValues(customerReport.cashOut) + sumValues(resellerReport.cashOut);
    const totalSell =
        sumValues(customerReport.sell) + sumValues(resellerReport.sell);
    const totalReturn =
        sumValues(customerReport.return) + sumValues(resellerReport.return);
                ...
                <div className="space-y-10">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                            ...
                            <Card className="border-blue-200 bg-blue-50/50 dark:border-blue-500/20 dark:bg-blue-500/10">
                                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                    <CardTitle className="text-sm font-medium">
                                        Global Nett Sell
                                    </CardTitle>
