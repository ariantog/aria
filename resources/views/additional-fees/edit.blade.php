...
@section('title', 'Edit Additional Fee')

@section('content')
@php
$breadcrumbs = [
    ['title' => 'Additional Fees', 'href' => route('additional-fees.index')],
    ['title' => 'Edit Fee', 'href' => route('additional-fees.edit', $additional_fee->id)],
];
@endphp

<div class="flex flex-col gap-4 p-3 sm:p-4">
    <div>
        <h2 class="text-2xl font-bold tracking-tight text-gray-900">Edit Additional Fee</h2>
    </div>

    <div class="max-w-lg overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        <form method="POST" action="{{ route('additional-fees.update', $additional_fee->id) }}" class="space-y-4 p-6">
            @csrf
            @method('PUT')
            <div>
                <label class="mb-1 block text-sm font-medium text-gray-700">Name <span class="text-red-500">*</span></label>
                <input type="text" name="name" value="{{ old('name', $additional_fee->name) }}" required
            ...
            <div>
                ...
                <select name="type" required class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500">
                    <option value="nominal" @selected(old('type', $additional_fee->type) === 'nominal')>Nominal</option>
                    <option value="percent" @selected(old('type', $additional_fee->type) === 'percent')>Percent</option>
                </select>
                @error('type')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="mb-1 block text-sm font-medium text-gray-700">Value <span class="text-red-500">*</span></label>
                <input type="number" step="any" min="0" name="value" value="{{ old('value', $additional_fee->value) }}" required
                       class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500">
