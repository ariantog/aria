...
$legacySku = trim((string) ($item->legacy_code ?? ''));
@endphp
            ...
            <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                <div class="flex items-center justify-between border-b border-gray-100 bg-gray-50 px-6 py-4">
                    ...
                    <div class="flex flex-wrap items-center justify-end gap-2">
                        <a href="{{ route('items.jubelio-search', ['item' => $item->id, 'q' => $item->code]) }}"
                           data-testid="jubelio-link-current-sku"
                           class="inline-flex items-center gap-2 rounded-xl bg-green-600 px-4 py-2 text-sm font-medium text-white hover:bg-green-700">
                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                            Link Item
                        </a>
                        @if($legacySku !== '')
                        <a href="{{ route('items.jubelio-search', ['item' => $item->id, 'q' => $legacySku]) }}"
                           data-testid="jubelio-link-legacy-sku"
                           class="inline-flex items-center gap-2 rounded-xl border border-green-600 bg-white px-4 py-2 text-sm font-medium text-green-700 hover:bg-green-50">
                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                            Link Legacy SKU
                        </a>
                        @endif
