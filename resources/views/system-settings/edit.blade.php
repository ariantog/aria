...
$isTutupBuku = $setting->slug === 'tutup_buku';
$isAccountSetting = in_array($setting->slug, ['sell_100', 'ongkir']);
$isNumericSetting = in_array($setting->slug, ['batas_cuti_tahunan', 'batas_cuti_sakit']);
$tutupBukuOptions = [3, 4, 5, 6, 7, 8, 9, 10, 20, 28];
...
<div class="flex flex-col gap-4 p-3 sm:p-4">
    <div>
        ...
        <p class="mt-0.5 text-sm text-gray-500">Update category, name and value. The slug cannot be changed.</p>
    </div>
        ...
        <form method="POST" action="{{ route('system-settings.update', $setting->id) }}" class="space-y-4 p-6">
            ...
            @if($isTutupBuku)
                <div>
                    ...
                    <select name="value" required class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500">
                        @foreach($tutupBukuOptions as $day)
                            <option value="{{ $day }}" @selected((string) $currentValue === (string) $day)>Tanggal {{ $day }}</option>
                        @endforeach
                    </select>
                    @error('value')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                </div>
            @elseif($isAccountSetting)
                <div x-data="asyncCombobox({
                        endpoint: '/addrbook/account',
                        hiddenField: 'setting-value-account',
                        placeholder: 'Select account...',
                        initial: @js($currentValue ? ['id' => $currentValue, 'name' => $currentValue] : null),
