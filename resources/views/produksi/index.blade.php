...
<div class="flex flex-col gap-2 overflow-x-auto p-2 sm:p-4" x-data="produksiIndex()">
    <div class="mb-4 flex flex-col items-start justify-between gap-2 sm:flex-row sm:items-center">
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-zinc-900">Produksi</h2>
            <p class="mt-1 text-sm text-zinc-500">List of production records at the cutting stage.</p>
        </div>
            ...
            <table class="w-full border-separate border-spacing-0 text-left">
                <thead class="bg-zinc-50">
                    <tr>
                        ...
                        <th class="sticky top-0 z-20 border-b bg-zinc-50 px-2 py-3 text-[11px] font-bold uppercase tracking-wider text-zinc-500">{{ $h }}</th>
                        @endforeach
                        <th class="sticky top-0 z-20 border-b bg-zinc-50 px-2 py-3 text-center text-[11px] font-bold uppercase tracking-wider text-zinc-500">Jahit</th>
                        <th class="sticky top-0 z-20 border-b bg-zinc-50 px-2 py-3 text-center text-[11px] font-bold uppercase tracking-wider text-zinc-500">Action</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr class="transition-colors hover:bg-zinc-50/50">
                        ...
                        <td class="px-2 py-1 text-[11px] font-bold tabular-nums text-zinc-900">{{ $p->quantity }}</td>
                        <td class="px-2 py-1 text-[11px] text-zinc-600">{{ $p->surat_jalan_potong ?: '-' }}</td>
                        <td class="px-2 py-1">
                            <div class="flex flex-col items-center justify-center rounded-md border border-zinc-200 bg-zinc-50 p-1">
                                <span class="mb-0.5 text-[9px] font-medium text-zinc-500">{{ $p->potong_date ? \Carbon\Carbon::parse($p->potong_date)->translatedFormat('d M Y') : '-' }}</span>
                                @if($p->potong)<span class="max-w-[100px] truncate rounded border border-zinc-100 bg-white px-1.5 py-0.5 text-[9px] font-bold text-zinc-900 shadow-sm" title="{{ $p->potong->name }}">{{ $p->potong->name }}</span>@endif
                            </div>
                        </td>
                        <td class="px-2 py-1"><span class="rounded border border-gray-200 bg-zinc-50 px-1 font-mono text-[9px]">{{ $p->size->name ?? '-' }}</span></td>
                        <td class="px-2 py-1 text-[11px] font-bold text-zinc-600">{{ $p->warna ?: '-' }}</td>
                        <td class="max-w-[150px] break-words px-2 py-1 text-[11px] font-bold leading-tight text-zinc-600">{{ $p->customer ?: '-' }}</td>
                        <td class="px-2 py-1 text-center">
                            @if($p->jahit_date)
                            <div class="flex flex-col items-center justify-center rounded-md border border-zinc-200 bg-zinc-50 p-1">
                                <span class="mb-0.5 text-[9px] font-medium text-zinc-500">{{ \Carbon\Carbon::parse($p->jahit_date)->translatedFormat('d M Y') }}</span>
                                @if($p->jahit)<span class="max-w-[100px] truncate rounded border border-zinc-100 bg-white px-1.5 py-0.5 text-[9px] font-bold text-zinc-900 shadow-sm" title="{{ $p->jahit->name }}">{{ $p->jahit->name }}</span>@endif
                            </div>
                        ...
                        <td class="px-2 py-1 text-center">
                            <div class="flex items-center justify-center gap-1">
                                ...
                                <a href="{{ url('produksi/'.$p->id.'/edit') }}" title="Edit" class="flex h-6 w-6 items-center justify-center rounded-md border border-zinc-200 text-zinc-600 hover:bg-zinc-50">
                    ...
                    @empty
                    <tr><td colspan="10" class="px-6 py-12 text-center text-sm text-zinc-500">No production records found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div x-show="assignOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" @keydown.escape.window="assignOpen=false">
        <div @click.away="assignOpen=false" class="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
            ...
            <p class="mt-2 text-sm text-zinc-500">Select a worker to assign to the <span class="font-semibold text-emerald-600">Jahit</span> stage.</p>
            <div class="mt-4 flex flex-col gap-1 rounded-lg border border-zinc-200 bg-zinc-50 p-3">
                <span class="text-xs font-bold uppercase tracking-wider text-zinc-400">Selected Item</span>
                <span class="text-sm font-semibold text-zinc-900" x-text="sel.name"></span>
                <span class="font-mono text-xs text-zinc-500">Kitir: <span x-text="sel.serial"></span></span>
            </div>
            <form :action="assignAction" method="POST" class="mt-4 space-y-3">
                @csrf @method('PATCH')
                <label class="block text-sm font-semibold text-zinc-900">Worker Name</label>
