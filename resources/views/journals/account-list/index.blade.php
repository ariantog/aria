...
<div class="flex flex-col gap-4 overflow-x-auto p-4" x-data="accListCrud()">
    <div class="mb-4 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-gray-900">Account List</h2>
            <p class="mt-1 text-sm text-gray-500">Manage ledger accounts and view their journals.</p>
        </div>
    ...
    <form method="GET" action="{{ route('account-list.index') }}" class="mb-2 flex max-w-2xl flex-col items-center gap-3 sm:flex-row">
        <div class="relative w-full sm:max-w-sm">
            ...
            <input name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Search accounts..." class="w-full rounded-md border border-gray-300 bg-gray-50 py-2 pl-9 pr-3 text-sm">
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50/50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Operation</th>
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                    ...
                    <tr class="hover:bg-gray-50/50">
                        ...
                        <td class="whitespace-nowrap px-6 py-4">
                            <div class="text-sm font-medium text-gray-600">{{ $acc->operation->name ?? '' }}@if(!$acc->operation)<span class="text-red-400">Uncategorized</span>@endif</div>
                        </td>
                    ...
                    @empty
                    <tr><td colspan="3" class="px-6 py-12 text-center text-sm text-gray-500">No accounts found. Create one to start recording journals.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
