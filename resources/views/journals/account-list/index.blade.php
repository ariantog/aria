...
@section('title', 'Account List (Buku Besar)')
...
<div class="flex flex-col gap-4 overflow-x-auto p-4" x-data="accListCrud()">
    <div class="mb-4 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-zinc-900">Account List</h2>
            <p class="mt-1 text-sm text-zinc-500">Manage ledger accounts and view their journals.</p>
        </div>
    ...
    <form method="GET" action="{{ route('account-list.index') }}" class="mb-2 flex max-w-2xl flex-col items-center gap-3 sm:flex-row">
        <div class="relative w-full sm:max-w-sm">
            ...
            <input name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Search accounts..." class="w-full rounded-md border border-gray-300 bg-zinc-50 py-2 pl-9 pr-3 text-sm">
        </div>
        <div class="w-full sm:w-64">
            <select name="operation_id" onchange="this.form.submit()" class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm">
                <option value="">All Operations</option>
                @foreach($operations as $op)
                <option value="{{ $op->id }}" @selected((string)($filters['operation_id'] ?? '') === (string)$op->id)>{{ $op->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-zinc-200">
                <thead class="bg-zinc-50/50">
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-zinc-500">Account Name</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-zinc-500">Operation</th>
                ...
                <tbody class="divide-y divide-zinc-200 bg-white">
                    ...
                    <tr class="hover:bg-zinc-50/50">
                        <td class="whitespace-nowrap px-6 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <a href="{{ route('account-list.ledger', $acc->id) }}" class="text-sm font-bold text-zinc-900 hover:text-blue-600">{{ $acc->name }}</a>
                                    <div class="text-xs text-zinc-500">{{ $acc->description ?: 'No description' }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="whitespace-nowrap px-6 py-4">
                            <div class="text-sm font-medium text-zinc-600">{{ $acc->operation->name ?? '' }}@if(!$acc->operation)<span class="text-red-400">Uncategorized</span>@endif</div>
                        </td>
                        <td class="whitespace-nowrap px-6 py-4 text-right text-sm">
                            <div class="flex justify-end gap-2">
                                ...
                                <button @click="openEdit({{ $acc->id }}, @js($acc->name), @js($acc->description), @js((string)$acc->operation_id))" class="flex h-8 w-8 items-center justify-center rounded text-zinc-400 hover:text-zinc-900">
                                ...
                                <form method="POST" action="{{ route('account-list.destroy', $acc->id) }}" onsubmit="return confirm('Are you sure you want to delete this account?')">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="flex h-8 w-8 items-center justify-center rounded text-zinc-400 hover:text-red-600">
                    ...
                    @empty
                    <tr><td colspan="3" class="px-6 py-12 text-center text-sm text-zinc-500">No accounts found. Create one to start recording journals.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div x-show="isOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" @keydown.escape.window="isOpen=false">
        <div @click.away="isOpen=false" class="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
            ...
            <form :action="formAction" method="POST" class="space-y-4">
                ...
                <div class="space-y-2">
                    <label class="text-sm font-medium">Operation Category</label>
                    <select name="operation_id" x-model="operationId" required class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm">
