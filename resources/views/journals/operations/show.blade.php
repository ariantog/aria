...
<div class="flex flex-col gap-4 overflow-x-auto p-4" x-data="accCrud()">
    <div class="mb-4 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-2xl font-bold tracking-tight text-zinc-900">Operation: {{ $operation->name }}</h2>
                <p class="mt-1 text-sm text-zinc-500">Account list bound to this operation.</p>
            </div>
        </div>
    ...
    <form method="GET" action="{{ route('operations.show', $operation->id) }}" class="relative mb-2 max-w-sm">
        ...
        <input name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Search accounts in operation..." class="w-full rounded-md border border-gray-300 bg-zinc-50 py-2 pl-9 pr-3 text-sm">
    </form>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-zinc-200">
                ...
                <tbody class="divide-y divide-zinc-200 bg-white">
                    ...
                    <tr class="hover:bg-zinc-50/50">
                        ...
                        <td class="whitespace-nowrap px-6 py-4 text-right text-sm">
                            <div class="flex justify-end gap-2">
                                ...
                                <button @click="openEdit({{ $acc->id }}, @js($acc->name), @js($acc->description))" class="flex h-8 w-8 items-center justify-center rounded text-zinc-400 hover:text-zinc-900">
                    ...
                    @empty
                    <tr><td colspan="2" class="px-6 py-12 text-center text-sm text-zinc-500">No accounts found in this operation yet.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
