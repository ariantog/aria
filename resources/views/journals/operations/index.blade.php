...
<div class="flex flex-col gap-4 overflow-x-auto p-4" x-data="opCrud()">
    <div class="mb-4 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-zinc-900">Operations</h2>
            <p class="mt-1 text-sm text-zinc-500">Manage journal operation categories.</p>
        </div>
    ...
    <form method="GET" action="{{ route('operations.index') }}" class="relative mb-2 max-w-sm">
        ...
        <input name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Search operations..." class="w-full rounded-md border border-gray-300 bg-zinc-50 py-2 pl-9 pr-3 text-sm">
    </form>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-zinc-200">
                <thead class="bg-zinc-50/50">
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-zinc-500">Operation Name</th>
                        <th class="px-6 py-4 text-center text-xs font-semibold uppercase tracking-wider text-zinc-500">Actions</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-zinc-200 bg-white">
                        ...
                        <td class="whitespace-nowrap px-6 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div class="text-xs text-zinc-500">{{ $op->description ?: 'No description' }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="whitespace-nowrap px-6 py-4 text-center text-sm">
                            <div class="flex justify-center gap-2">
                                <button @click="openEdit({{ $op->id }}, @js($op->name), @js($op->description))" class="flex h-8 w-8 items-center justify-center rounded text-zinc-400 hover:text-zinc-900">
                    ...
                    @empty
                    <tr><td colspan="2" class="px-6 py-12 text-center text-sm text-zinc-500">No operations found. Get started by adding a new operation.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
