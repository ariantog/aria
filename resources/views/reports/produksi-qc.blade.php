...
$breadcrumbs = [
    ...
    ['title' => 'Reports', 'href' => '#'],
    ['title' => 'Statistik QC', 'href' => route('reports.produksi-qc')],
];
...
<div class="flex flex-col gap-4 p-4">
    ...
    @include('reports.partials.month-year-filter', [
        'action' => route('reports.produksi-qc'),
        'filters' => $filters,
        'yearList' => $yearList,
    ])
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
        <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
            <p class="text-sm text-gray-500">Total Kitir QC'd</p>
    ...
    <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-gray-50 text-left text-xs uppercase text-gray-500">
                    <tr>
                        <th class="px-4 py-3 font-medium">QC Worker</th>
                        ...
                        <th class="px-4 py-3 text-right font-medium">Avg Potong→QC (days)</th>
                        <th class="px-4 py-3 text-right font-medium">Avg Setor→QC (days)</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50/50">
                        ...
                        <td class="px-4 py-3 text-right font-semibold tabular-nums text-indigo-600">{{ number_format($row->total_qty) }}</td>
                    ...
                    @empty
                    <tr><td colspan="6" class="px-4 py-10 text-center text-gray-500">No QC assignments in this period.</td></tr>
                    @endforelse
                    @if($workerSummary->isNotEmpty())
                    <tr class="bg-gray-50 font-bold">
                        <td class="px-4 py-3">Total</td>
                        <td class="px-4 py-3 text-right tabular-nums">{{ number_format($totalKitir) }}</td>
                        <td class="px-4 py-3 text-right tabular-nums text-indigo-600">{{ number_format($totalQty) }}</td>
