                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-3 py-2.5 text-center">
                                <div class="flex justify-center gap-1">
                                    ...
                                    @endif
                                    @if($can['edit'] && $a->deleted_at)
                                    <button type="button" onclick="restoreAddrbook({{ $a->id }})" title="Restore" class="inline-flex h-8 w-8 items-center justify-center rounded text-gray-500 hover:bg-gray-100 hover:text-emerald-600">
...
async function restoreAddrbook(id) {
    if (!confirm('Restore this deleted contact?')) return;
    const res = await fetch(`/addrbook/${id}/restore`, {
        method: 'POST',
