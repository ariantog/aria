...
<div class="flex flex-col gap-4 p-4">
    ...
    @if($typeTags->isEmpty())
        <div class="rounded-xl border border-gray-200 bg-white p-8 text-center text-gray-500">
            No TYPE tags found for asset lancar restock. Create TYPE tags under Stuff → Tags
            (exclude manufactured item types).
        </div>
    @else
        @include('restock.partials.type-tabs')

        @if($activeTypeTag && $typeSheet)
            @if($typeSheet['sku_count'] === 0)
            ...
            @else
                @php
                    $sheet = $typeSheet['sheet'];
                    $imageUrl = $sheet?->image_url ?? asset('images/default-item.png');
                @endphp
                    ...
                    <table class="min-w-full divide-y divide-gray-200 text-sm">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-right font-medium text-gray-600">Parents</th>
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span class="font-medium text-gray-900">{{ $activeTypeTag->name }}</span>
                                    </div>
                                </td>
                                <td class="px-4 py-3 text-right tabular-nums text-gray-600">{{ $typeSheet['parent_pcode_count'] }}</td>
                                <td class="px-4 py-3 text-right tabular-nums text-gray-600">{{ $typeSheet['sku_count'] }}</td>
                                @if($sheet)
                                    <td class="px-4 py-3 text-right tabular-nums">{{ number_format($typeSheet['totals']['restock']) }}</td>
                                    <td class="px-4 py-3 text-right tabular-nums">{{ number_format($typeSheet['totals']['production']) }}</td>
                                    <td class="px-4 py-3 text-right tabular-nums">{{ number_format($typeSheet['totals']['shipped']) }}</td>
                                    <td class="px-4 py-3 text-center">
                                        @if($typeSheet['urgent_count'] > 0)
                                            <span class="inline-flex rounded-full bg-red-100 px-2 py-0.5 text-xs font-medium text-red-700">{{ $typeSheet['urgent_count'] }}</span>
                                        @else
                                    ...
                                    <td class="px-4 py-3 text-right">
                                        <a href="{{ route('restock.sheets.show', $sheet) }}"
                                           class="font-medium text-blue-600 hover:text-blue-800">Open sheet</a>
                                    </td>
                                @else
                                    <td colspan="4" class="px-4 py-3 text-gray-400">Not tracked yet</td>
                                    <td class="px-4 py-3 text-right">
                                        @if($canCreateSheet ?? false)
                                            <form method="POST" action="{{ route('restock.sheets.store', $activeTypeTag) }}" class="inline">
                                                @csrf
                                                <button type="submit" class="font-medium text-blue-600 hover:text-blue-800">Start tracking</button>
                                            </form>
                                        @endif
                                    </td>
                                @endif
