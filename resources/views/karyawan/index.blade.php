...
<div class="flex flex-col gap-4 overflow-x-auto p-4">
    <div class="mb-4 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        <div>
            <h1 class="mb-1 text-3xl font-bold tracking-tight text-gray-900">Daftar Karyawan</h1>
            <p class="text-gray-500">Kelola data karyawan, gaji bulanan, dan cuti.</p>
        </div>
    ...
    <form method="GET" action="{{ route('karyawan.index') }}" class="flex flex-wrap items-end gap-2 rounded-xl border border-gray-200 bg-white p-3">
        <div class="flex flex-col gap-1">
            <label class="text-xs font-medium uppercase text-gray-500">Cari Nama</label>
            <input type="text" name="name" value="{{ $filters['name'] ?? '' }}" placeholder="Nama karyawan…" class="rounded-md border border-gray-300 px-2.5 py-1.5 text-sm">
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm">
                <thead class="border-b border-gray-200 bg-gray-50 text-xs uppercase text-gray-500">
                    <tr>
                        <th class="px-6 py-4 font-bold tracking-wider">Nama / ID</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($karyawans as $item)
                        @php $upahPokok = $item->gajiSingle ? $item->gajiSingle->upah_pokok : 0; @endphp
                        <tr class="transition-colors hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="font-medium text-gray-900">
                                    ...
                                    @else <span class="text-xs italic text-gray-400">Belum dibuat</span> @endif
                                </div>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="font-medium text-gray-900">
                                    @if($item->gajiSingle) Rp {{ format_amount($upahPokok, 0) }}
                                    @else <span class="text-xs italic text-gray-400">Belum dibuat</span> @endif
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end gap-2 text-xs">
                                    ...
                                    <a href="{{ route('karyawan.edit', $item->id) }}" class="inline-flex items-center rounded-md p-2 text-gray-400 hover:bg-blue-50 hover:text-blue-500">
                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </a>
                                    @endif
                                    @if($isSuper || $user->can('karyawan-delete'))
                                    <form method="POST" action="{{ route('karyawan.destroy', $item->id) }}" onsubmit="return confirm('Yakin hapus karyawan ini?')">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="inline-flex items-center rounded-md p-2 text-gray-400 hover:bg-red-50 hover:text-red-500">
                    ...
                    @empty
                        <tr><td colspan="6" class="px-6 py-12 text-center text-gray-500">Belum ada data karyawan.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
